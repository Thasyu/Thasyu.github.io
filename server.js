...
// Session (小規模用メモリストア。複数インスタンスなら Redis 等推奨)
app.use(
  session({
...
app.get("/registerthanks", (req, res) => res.sendFile(path.join(__dirname, "public", "registerthanks.html")));
...
app.get("/diagnosis/diagnosis.html", (req, res) => {
  if (!req.session.user) return res.redirect("/login");
  res.sendFile(path.join(__dirname, "public", "diagnosis", "diagnosis.html"));
});
...
app.get("/inquiry", csrfProtection, (req, res) => {
  if (!req.session.user) return res.redirect("/login");
  let html = fs.readFileSync(path.join(__dirname, "public", "inquiry", "inquiry.html"), "utf8");
...
app.post("/inquiry", csrfProtection, (req, res) => {
  ...
  res.sendFile(path.join(__dirname, "public", "inquiry", "thanks.html"));
});
